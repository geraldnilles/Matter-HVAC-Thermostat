...
| `outdoor_temp` | sensor | Informational outdoor sensor °F (only when `outdoor_sensor` is configured; file is removed while the sensor is stale). Read by WebUI and MQTT daemon. **Never** folded into `current_temp`/`min_temp`/`max_temp` — excluded from HVAC control |

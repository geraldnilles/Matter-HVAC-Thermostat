...
| `test_mqtt.py` | MQTT bridge (`src/mqtt.py`): matterbridge-mqtt device protocol for the **Thermostat**, **Fan** and outdoor **TemperatureSensor** devices — retained config/state/subscribe payloads, unit conversion (hundredths of °C, `SystemModeEnum`, `FanControl` fanMode auto/on ⇄ Auto/High (`OffHighAuto` sequence), and `measuredValue`/`outdoorTemperature` from `outdoor_temp` when present), controller write handling and per-device echo suppression — driven with a recording fake client, so no broker is needed |
...
| `test_outdoor.py` | Informational outdoor sensor: `utils.get_outdoor_sensor` / `utils.partition_sensors` config parsing (outdoor MAC excluded from the room allowlist), `demo.py` outdoor scalar + history field, and `web.read_state`'s `outdoor_temp` / `outdoor_configured`. `src/sensor.py` is not imported (it needs `bleak`); its outdoor logic is built on the tested `utils` helpers |

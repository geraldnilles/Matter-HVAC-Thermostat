...
| `test_mqtt.py` | MQTT bridge (`src/mqtt.py`): matterbridge-mqtt device protocol — retained config/state/subscribe payloads, unit conversion (hundredths of °C, `SystemModeEnum`, and `outdoorTemperature` from `outdoor_temp` when present), controller write handling and echo suppression — driven with a recording fake client, so no broker is needed |

...
| `test_mqtt.py` | MQTT bridge (`src/mqtt.py`): matterbridge-mqtt device protocol for the **Thermostat**, **Fan** and outdoor **TemperatureSensor** devices — retained config/state/subscribe payloads, unit conversion (hundredths of °C, `SystemModeEnum`, `FanControl` fanMode auto/on ⇄ Off/High, and `measuredValue`/`outdoorTemperature` from `outdoor_temp` when present), controller write handling and per-device echo suppression — driven with a recording fake client, so no broker is needed |
